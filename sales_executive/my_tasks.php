...
// Falls back to 'Kavya Aruldas' if session is empty during your testing
$my_name = !empty($_SESSION['name']) ? $_SESSION['name'] : 'Kavya Aruldas'; 
...
$achieved_query = mysqli_query($conn, "SELECT SUM(deal_value) as total_achieved FROM crm_clients WHERE status = 'Won' AND DATE_FORMAT(created_at, '%Y-%m') = '$current_month' AND executive = '$my_name'");
        ...
        <div style="flex: 1; min-width: 300px;">
            ...
            <div class="target-grid" id="targetsContainer" style="display: none;">
                <?php if(empty($my_targets)): ?>
                    <div style="padding: 20px; background: white; border-radius: 12px; color: var(--text-muted); width: 100%;">No targets set for you in <?php echo date('F Y'); ?>.</div>
