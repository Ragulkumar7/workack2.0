...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // Clear out any stray output that could corrupt the JSON response
    ...
    try {
        ...
        if ($_POST['action'] === 'save_invoice') {
            ...
            // Convert to float to perfectly match the DECIMAL(15,2) in the DB
...
<head>
    ...
    <style>
        ...
        .invoice-card, .history-section { background: white; padding: 30px; border-radius: 12px; border: 1px solid var(--border-color); margin-bottom: 30px; }
...
<main id="mainContent" class="main-content">
    <div style="margin-bottom: 25px;">
        <h2 style="color:var(--theme-color); margin:0;">Invoice Creation</h2>
    ...
    <div class="invoice-card">
        <form id="invoiceForm">
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
                ...
                <div class="form-group"><label>Client</label><div style="display:flex; gap:5px;"><select name="client_id" id="client_id" required style="flex:1"><option value="">Select Client</option><?php if($clients) { mysqli_data_seek($clients, 0); while($row = mysqli_fetch_assoc($clients)) { echo "<option value='".$row['id']."'>".$row['client_name']."</option>"; } } ?></select><button type="button" style="background:var(--theme-color); color:white; border:none; border-radius:6px; padding:0 10px;" onclick="document.getElementById('addClientModal').style.display='flex'">+</button></div></div>
            ...
            <table style="width:100%; border-collapse:collapse; margin-top:20px;">
                ...
                <tbody id="itemsTableBody">
                    <tr class="item-row">
                        ...
                        <td style="padding:5px; text-align:center;"><button type="button" onclick="this.closest('tr').remove(); calculateGrandTotal();" style="color:red; background:none; border:none; cursor:pointer;">&times;</button></td>
                    </tr>
                </tbody>
            </table>
            ...
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:25px;">
                <button type="button" style="padding:10px 20px; border:1px solid #ddd; border-radius:8px; background:white; cursor:pointer;" onclick="location.reload()">Reset</button>
                <button type="button" onclick="submitInvoice()" id="saveBtn" class="btn-save"><i class="ph ph-paper-plane-right"></i> Send to CFO for Approval</button>
            </div>
        </form>
    </div>
    ...
    <div class="history-section">
        ...
        <table class="history-table">
            <thead><tr style="background:#f8fafc; font-size:12px; color:var(--text-muted);"><th style="padding:15px;">Invoice No</th><th style="padding:15px;">Client</th><th style="padding:15px;">Amount</th><th style="padding:15px;">Status</th><th style="padding:15px; text-align:center;">Actions</th></tr></thead>
            <tbody>
                <?php if($history): while($row = mysqli_fetch_assoc($history)): $isApp = ($row['status'] == 'Approved'); ?>
                <tr>
                    ...
                    <td style="padding:15px; text-align:center;">
                        <?php if($isApp): ?>
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <button onclick="prepareAndPrint('<?= $row['id'] ?>')" class="btn-action-icon btn-print"><i class="ph ph-printer"></i></button>
                                <button onclick="prepareAndPrint('<?= $row['id'] ?>')" class="btn-action-icon btn-download"><i class="ph ph-download-simple"></i></button>
                            </div>
                        <?php else: ?>
                            <span style="color:#94a3b8; font-size:12px;">Waiting</span>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endwhile; endif; ?>
...
<div class="modal-overlay" id="addClientModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:2000; align-items:center; justify-content:center;"><div style="background:white; padding:25px; border-radius:12px; width:350px;"><h3 style="margin-top:0; color:var(--theme-color);">Add Client</h3><input type="text" id="new_client_name" style="width:100%; border:1px solid #ddd; padding:10px; border-radius:6px; margin:15px 0; box-sizing:border-box;"><div style="display:flex; justify-content:flex-end; gap:10px;"><button onclick="document.getElementById('addClientModal').style.display='none'" style="padding:8px 15px; border-radius:6px; border:1px solid #ddd; background:white; cursor:pointer;">Cancel</button><button onclick="saveNewClient()" style="background:var(--theme-color); color:white; padding:8px 15px; border-radius:6px; border:none; cursor:pointer;">Save</button></div></div></div>
...
<script>
    function addRow() {
        ...
        tr.innerHTML = `<td style="padding:5px;"><input type="text" name="item_desc[]" required style="width:100%; border:1px solid #ddd; padding:8px; border-radius:4px; box-sizing:border-box;"></td><td style="padding:5px;"><input type="number" name="item_qty[]" class="item-qty" value="1" onchange="calculateRow(this)" style="width:100%; border:1px solid #ddd; padding:8px; border-radius:4px; box-sizing:border-box;"></td><td style="padding:5px;"><input type="number" name="item_rate[]" class="item-rate" step="0.01" onchange="calculateRow(this)" style="width:100%; border:1px solid #ddd; padding:8px; border-radius:4px; box-sizing:border-box;"></td><td style="padding:5px;"><input type="number" name="item_disc_val[]" class="item-disc" value="0" onchange="calculateRow(this)" style="width:100%; border:1px solid #ddd; padding:8px; border-radius:4px; box-sizing:border-box;"></td><td style="padding:5px;"><input type="number" name="item_total[]" class="item-total" readonly style="width:100%; border:1px solid #ddd; padding:8px; border-radius:4px; background:#f8fafc; box-sizing:border-box;"></td><td style="padding:5px; text-align:center;"><button type="button" onclick="this.closest('tr').remove(); calculateGrandTotal();" style="color:red; background:none; border:none; cursor:pointer;">&times;</button></td>`;
    ...
    // STRICT ERROR CATCHING: STOPS "SENDING..." IF SERVER FAILS
        ...
        .then(async response => {
            ...
            try {
                ...
                if(data.status === 'success') {
                    ...
                    btn.disabled = false; btn.innerHTML = '<i class="ph ph-paper-plane-right"></i> Send to CFO';
                }
            } catch (err) {
                // Shows the exact PHP/SQL error if JSON fails
                Swal.fire('Schema Error', 'Please run the SQL drop/create queries. System says: ' + textResponse.substring(0, 100), 'error');
                btn.disabled = false; btn.innerHTML = '<i class="ph ph-paper-plane-right"></i> Send to CFO';
            }
        }).catch(err => {
    ...
    function saveNewClient() {
        ...
        fetch('', { method: 'POST', body: fd }).then(r => r.json()).then(res => { if(res.status === 'success') location.reload(); });
    }
</script>
</body>
</html>