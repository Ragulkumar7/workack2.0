...
<head>
    ...
    <style>
:root { --primary-color: #dd1313ff; --primary-hover: #4752c4; --bg-body: #f0f2f5fe; --bg-white: #fffffff6; --text-main: #111827; --text-secondary: #17191dff; --border-color: #E5E7EB; --incoming-msg-bg: #ffffff; --outgoing-msg-bg: #5865F2; --outgoing-msg-text: #ffffff; --danger-color: #EF4444; --success-color: #10B981; --warning-color: #F59E0B; --sidebar-width: 380px; --header-height: 80px; --input-area-height: 90px; }
* { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI', 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; }
body { background-color:var(--bg-body); height:100vh; width:100vw; display:flex; align-items:center; justify-content:center; overflow:hidden; }
.app-container { width:100%; height:100%; max-width:1920px; background:var(--bg-white); display:flex; box-shadow:0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); overflow:hidden; position:relative; }
.sidebar { width:var(--sidebar-width); border-right:1px solid var(--border-color); display:flex; flex-direction:column; background-color:var(--bg-white); flex-shrink:0; height:100%; z-index:20; transition:transform 0.3s ease; }
.sidebar-header { height:auto; min-height:130px; padding:24px; display:flex; flex-direction:column; justify-content:flex-start; border-bottom:1px solid var(--border-color); background-color:var(--bg-white); flex-shrink:0; }
...
.chat-area { flex:1; display:flex; flex-direction:column; background-color:#F8FAFC; position:relative; z-index:10; height:100%; min-width:0; }
...
.video-container { position:relative; flex:1; width:100%; overflow:hidden; background-color:#202124; }
.remote-placeholder { position:absolute; top:0; left:0; width:100%; height:100%; display:flex; align-items:center; justify-content:center; z-index:0; }
#remoteInitials { font-size:8rem; color:#ffffff; font-weight:700; letter-spacing:4px; text-transform:uppercase; }
#remoteVideo { width:100%; height:100%; object-fit:cover; z-index:1; background:transparent; }
.local-video-wrapper { position:absolute; bottom:20px; right:20px; width:200px; height:150px; background:#333; border-radius:16px; overflow:hidden; box-shadow:0 8px 20px rgba(0,0,0,0.5); border:2px solid rgba(255,255,255,0.2); z-index:2001; transition:all 0.3s ease; }
...
@media (max-width: 768px) {
    ...
    .sidebar { position:absolute; z-index:50; height:100%; width:100%; transform:translateX(0); }
    ...
    <div class="app-container">
        ...
        <main class="chat-area">
            <header class="chat-top-bar">
                ...
                <div class="action-icons">
                    <i class="ri-phone-line action-icon" id="openConfCallBtn" title="Start Conference Call"></i>
        ...
        <div class="video-call-overlay" id="videoCallOverlay">
            ...
            <div class="video-container">
                <!-- Google Meet Style Placeholder -->
                <div class="remote-placeholder" id="remotePlaceholder">
                    <span id="remoteInitials">AL</span>
                </div>
                
                <!-- Remote Video (Empty/Transparent until stream connects) -->
        ...
        <div class="modal-overlay" id="confModal">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 id="modalTitle">Add to Video Call</h3>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn-primary" id="startConfBtn">Start Video Call</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const contacts = [
            ...
            },
            {
                ...
                previewType: "document", // custom flag
        ...
        let currentChatId = 1; // Default active chat (Anthony Lewis)
        let localStream = null;
        let callTimerInterval = null;
        let callSeconds = 0;
        let selectedConfContacts = new Set();
        let modalSource = 'video'; // 'video' or 'audio' to distinguish context
        ...
        const openConfCallBtn = document.getElementById('openConfCallBtn');
        ...
        function renderMessages() {
            ...
            contact.messages.forEach(msg => {
                ...
                const avatarUrl = msg.type === 'outgoing' 
                    ? 'https://picsum.photos/seed/me/200/200.jpg' // Current User avatar
                    : contact.avatar;
        ...
        // Helper to finalize sending (common logic for text and file)
        function finalizeSending(contact) {
            // Clear typing status from contact if they were typing
            if(contact.typing) {
        ...
        function simulateReply(contact) {
            // Show "Typing..." in sidebar and header after short delay
            setTimeout(() => {
                contact.typing = true;
                renderChatList();
                updateHeader();
            ...
            setTimeout(() => {
                ...
                // If currently looking at this chat, render it
                ...
                // Mark previous outgoing message as read
        ...
        async function startVideoCall(isConference = false, participants = []) {
            const contact = contacts.find(c => c.id === currentChatId);
            if(!contact && !isConference) return;

            // Update Call UI text based on type
            if(isConference) {
                const names = participants.map(id => contacts.find(c => c.id == id)?.name).join(', ');
                callUserName.textContent = "Conference: " + (names || contact?.name || "Group");
                
                // Set initials for Conference
        ...
        function endCall() {
            ...
            remoteVideo.srcObject = null; // Clear remote
        ...
        function openConfModal(source = 'video') {
            modalSource = source;
            selectedConfContacts.clear();
            
            // Pre-select current chat user
            ...
            if(source === 'video') {
                ...
                modalTitle.textContent = "Start Audio Call";
                startConfBtn.textContent = "Start Audio Call";
            }
        ...
        function toggleConfSelection(id, event) {
            // Prevent triggering when clicking directly on checkbox (handled by visual toggle)
        ...
        function handleStartConference() {
            ...
            closeConfModalFunc();
            startVideoCall(true, Array.from(selectedConfContacts));
        }
        ...
        // Modified to open modal instead of direct call
        ...
        // Conference Call Event Listeners
        openConfCallBtn.addEventListener('click', () => openConfModal('audio')); // Using modal for phone too
        ...
        // --- Initialization ---
        // Initial Setup
        contacts[0].typing = false; // Reset Anthony typing for demo purposes initially
        renderChatList();
        renderMessages();
        updateHeader();
        
        // Start the "Typing" simulation for Anthony after 1 second to match image feel
        setTimeout(() => {
